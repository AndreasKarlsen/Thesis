...
Tracking granularity is described in \bsref{sec:tracking}. The relationship between transactions and variables i.e. what and how variables should be tracked is addressed in \bsref{subsec:rec_transactions_variables}. Strong or weak atomicity is described in \bsref{sec:design_strong_weak_atomicity}. The different types of side-effects and how they are handled is discussed in \bsref{sec:side-effects}. Conditional synchronization is described in \bsref{sec:req_conditional} and nesting in \bsref{sec:stm_req_nesting}. The last requirement is opacity which is described in \bsref{sec:stm_req_opacity}. Finally, a summary of the requirements described in this chapter is presented in \bsref{sec:sum_of_req}.\toby{Skal alle henvisninger til sektionerne indeholde ordet requirement? det gør det ikke nu}

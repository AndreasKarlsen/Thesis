...
The \ac{STM} library uses special types for transactional variables, requiring the programmer to access the \bscode{Value} property whenever a assignment is to take place. Implicit conversion ensures that a object of one of the special \ac{STM} can, in most cases, be used as if it was an object of the type its wrapping. However this is not possible in all cases. \bsref{lst:lib_implicit_conversion} shows a equality comparison of two transactional variables using the \ac{STM} library. The comparison on line \ref{line:lib_equality} requires the programmer to access the \bscode{Value} property of the \bscode{TMVar} objects as implicit conversion will not be used if the \bscode{TMVars}'s are compared directly, resulting in reference comparison of the \bscode{TMVar} objects producing a incorrect result. A similar problem exist when calling a method on a \bscode{TMVar} object, implicit conversion does not allow methods of the wrapped type to be called on the \bscode{TMVar} directly. Such cases reduce the orthogonality of the \ac{STM} library. The \ac{STM} library however still benefits from the advantages provided by \ac{STM} and is place just above locking on the spectrum.

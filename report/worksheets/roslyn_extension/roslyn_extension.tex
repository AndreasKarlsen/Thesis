...
\begin{enumerate}
	...
	\item $T$ $\quad->\quad$ $T$
\end{enumerate} 
where $T$ is some type, \bscode{atomic} $T$ is $T$'s corresponding \ac{STM} type and $T$ $\quad->\quad$ \bscode{atomic} $T$ describes passing an argument of type $T$ into a parameter of the \ac{STM} type corresponding to $T$. The third and fourth cases are handle by the C\# compiler so transformation must only be applied in cases one and two.
...
\kasper[inline]{Special handling for atomic out parameters}
\subsection{Retry}

...
\begin{enumerate}
	...
	\item $T$ $\quad->\quad$ $T$
\end{enumerate} 
where $T$ $\quad->\quad$ \bscode{atomic} $T$ describes passing an argument of type $T$ into a parameter of the \ac{STM} type corresponding to $T$. The fourth case is handled by the C\# compiler so transformation must only be applied in cases 1-3.

\bsref{lst:before_atomic_ref} shows an example containing three method calls, each of which fall in one of the categories which require transformation. Applying the transformation to the exampled presented in \bsref{lst:before_atomic_ref} produces a syntax tree representing the code shown in \bsref{lst:after_atomic_ref}. For each argument node the transformation goes through the following steps:

...
Supporting \bscode{atomic} \bscode{ref} and \bscode{out} parameters, as described in \bsref{subsec:stm_desgin_ref_parameters} and \bsref{subsec:stm_desgin_out_parameters}, presents a problem due to the type mismatch as a result of transforming the parameters. \bscode{ref} and \bscode{out} parameters require the argument to be a assignable variable of the same type as the parameter. As the type of the parameter is transformed, a variable of type $T$ can not be passed directly as \bscode{ref} or \bscode{out} to a \bscode{atomic} parameter of type $T$.

Four different cases exist for passing \bscode{ref} and \bscode{out} parameters. There are: 
\begin{enumerate}
...
\section{Testing}
Unit testing is employed to ensure the Roslyn extension fulfills the design decisions of \stmnamesp and the integration with existing language features described in \bsref{chap:stm_design}. Each \ac{STM} construct and each integration with an existing language feature is covered by at least a single unit test. Furthermore, integration with disallowed existing language features is tested, f.x. there is a unit test that ensures an error is produced if the programmer tries to declare a method with an atomic optional parameter. This results in a rather extensive test suite, which is valuable in relation to the correctness of all \ac{STM} constructs and integration with language features. Furthermore it is especially valuable in relation to maintenance and further development of the Roslyn extension, f.x. it allows for big code changes, as the test suite can be run afterwards to ensure the extension still works correctly. Additionally, Roslyn is still under heavy development, which means radical changes may happen. The test suite can beneficial to test if the extension still works, if new changes are made to Roslyn.

%maintenance espceially valuable
%allow to make big changes to code, without you have to worry if you maybe broke somehting

%ensure

%also in relation to maintaining the STM inte
%also when new stm constructs or langague features are added.
%especailly important for Roslyn, as it is an early project, which has frequent changes, and therefore important to see if the extension still works correctly.


%test setup (eller beskriv en case)
...
%Every STM construct is tested and all the stm construct integrations with exsisting langague features is tested.

%test suite of 29 tests. 

%The tests further help.
...
%also in relation to maintenance and further integration

To ensure the roslyn extension works correctly, unit testing is employed.

%Hvordan det er gjort?
	%lav en en initial fil og kør med csc, hvorefter output tjekkes for errors, warnings eller exception (og exit code), hvor til sidst der tjekkes at den compilede fil stemmerovens med en fil man selv har lavet , det beskriver hvordan det compilede resultat skal se ud.

%the different scenarios
	% integration between exisisting langague features, discovered in section bla bla (stm design, tror jeg)
...
%ensure that the stm integration with C# is correct and that it continues to do so when adding additonal features (i forhold til at integere med andre constructs/features eksisterence i sproget allerede).
	%also in relation to when new things are added to roslyn, then being able to pull from the git repository and ensure the stm integration remains working correctly.

%extensive tests suite

%adding a new feature or integration with another feature will not have unwanted consequnces on already implemented stm features
...
As described in \bsref{subsec:stm_desgin_ref_parameters} and \bsref{subsec:stm_desgin_out_parameters} the intended design was that assignment to transactional \bscode{ref/out} parameters take effect when the transaction in which the assignment takes place commits. This behavior hold for the case:
\begin{itemize}
	\item $\bscode{atomic} \quad T \quad->\quad \bscode{atomic} \quad T$
\end{itemize}
That is, the case involving a \bscode{atomic} argument and parameter. For the cases:
\begin{itemize}
	\item $T \quad -> \quad \bscode{atomic} \quad T$
	\item $\bscode{atomic} \quad T \quad -> \quad T$
\end{itemize}
the behavior is however different. Due to the type mismatch between the argument and parameter, a intermediate local variable is passed as the actual argument and the original argument is only assigned just after the call finishes. This conflicts with traditional C\# where any assignment to a \bscode{ref/out} parameter takes effect immediately\cite[p. 76]{sestoft2011c}. If the parameter is declared \bscode{atomic} and the method call is wrapped in a \bscode{atomic} block the impact of the difference in timing is negated as any assignments to atomic variables take effect when the transaction commits. However if the call is not wrapped in a atomic block the difference in timing can have unintended consequences, especially when considering concurrent execution. Due to this issue a number of ways for handling \bscode{atomic ref/out} parameters and arguments where investigate. These where:
\begin{enumerate}
	\item Warning the programmer if one of the two problematic cases is utilized
	\item Disallowing the two problematic cases described above
	\item Disallowing \bscode{atomic ref/out} parameters
\end{enumerate}
In C\#, passing a volatile field as \bscode{ref/out} result in the field being treated as non volatile within the method body\cite{csharpVolatileRef}. If such a operation is detected the compiler creates a warning informing the programmer of the change in semantics. The same approach could be taken whenever one of the two problematic cases described earlier are detected.

Disallowing \bscode{atomic ref/out} parameters, preventing the programmer from declaring \bscode{atomic ref/out} parameters, removes the timing issues present in the cases where a intermediate \bscode{atomic} variable is required. However the ability to pass a atomic variable by reference is lost and the case of passing a variable of type \bscode{atomic} $T$ into a parameter of type $T$ is still unsolved.

Disallowing \bscode{ref/out} the cases where both a \bscode{atomic} and non \bscode{atomic} variable/parameter is involved removes the cases where the desired timing can not be provided. However it will, for example, non longer be possible to pass a atomic integer into a \bsref{out} integer parameter such as that of \bscode{Int32.TryParse}. As a result the programmer is more restricted when using \bscode{atomic} variables.

For the initial prototype presented in this chapter the first option of warning the programmer of the change in semantics has been selected. This ensures that the change in semantics is not hidden from the programmer thereby allowing her to adapt the program. The impact of the feature will be closely observed throughout the evaluation with the aim of determining the final form of the feature, or even if the feature should be part of the language at all.

...
Despite of the many advantages of the selected approach, we also found disadvantages which we have to cope with. The following disadvantages was identified:
\begin{enumerate}
	...
	\item As we translate \bscode{atomic} fields, properties and local variables to a corresponding \ac{STM} type, the types need to be known. As C\# allows the programmer to utilize type inference for local variables, the type can be unknown at this point of compilation, thus making it impossible to identify which type the local variables should be translated to. To remedy this, we must infer the type before translation. Roslyn offers the possibility to evaluate the type an expression, which we use to infer the type of \bscode{atomic} local variables of no type. Consequently, we must perform some of the work, that the Roslyn compiler does at later stages of the compilation.
\end{enumerate}
...
As described in \bsref{subsec:stm_desgin_value_parameters} \stmnamesp support value parameters. Replacing the type of atomic parameters with the corresponding \ac{STM} type,as described in \bsref{subsec:roslyn_extension_parameters}, results in a type mismatch when calling a method with a \bscode{atomic} parameter of some type $T$ as the parameter is no longer of type $T$, but $T$'s corresponding \ac{STM} type. As a result transformation must be applied to arguments passes to a \bscode{atomic} parameter, ensuring that the argument represents a object of the required \ac{STM} type. The transformation of an argument to a \bscode{atomic} parameter goes trough the following steps:

\begin{enumerate}
	\item Determine the \ac{STM} type of the formal parameter to which the argument corresponds.
	\item Construct a object initialization expression creating a object of the previously determined \ac{STM} type, giving the argument expression as a argument to constructor of the object.
	\item Replace the argument with the constructed object initialization expression.
\end{enumerate}

\bsref{lst:before_atomic_argument} presents a example of calling a method with a \bscode{atomic} parameter, while \bsref{lst:after_atomic_argument} shows the result of applying the transformation. Line \ref{line:aa_param_before} is transformed as described in \bsref{subsec:roslyn_extension_parameters}. Line \ref{line:aa_before} in \bsref{lst:before_atomic_argument} is transformed to line \ref{line:aa_after} in \bsref{lst:after_atomic_argument}. The argument to the \bscode{atomic} parameter is replaced with a object initialization expression creating a new \bscode{TMInt} object which wrapped value is initialized using the original argument.
...
Supporting \bscode{atomic} \bscode{ref} and \bscode{out} parameters, as described in \bsref{subsec:stm_desgin_ref_parameters} and \bsref{subsec:stm_desgin_out_parameters}, presents a problem due to the type missmatch as a result of transforming the parameters. \bscode{ref} and \bscode{out} parameters require the argument to be either a assignable variable of the same type as the parameter. As the type of the parameter is transformed, a variable of type $T$ can not be passed directly as \bscode{ref} or \bscode{out} to a \bscode{atomic} parameter of type $T$.

Four different cases exist for passing \bsref{ref} and \bscode{out} parameters. There are:
\begin{enumerate}
	\item $T$ $\quad->\quad$ \bscode{atomic} $T$
	\item \bscode{atomic} $T$ $\quad->\quad$ $T$
	\item \bscode{atomic} $T$ $\quad->\quad$ \bscode{atomic} $T$
	\item $T$ $\quad->\quad$ $T$
\end{enumerate} 
where $T$ $\quad->\quad$ \bscode{atomic} $T$ describes passing an argument of type $T$ into a parameter of the \ac{STM} type corresponding to $T$. The fourth case is handle by the C\# compiler so transformation must only be applied in cases 1-3.
...
\bsref{lst:before_atomic_ref} shows a example containing method three method calls each of which which fall in one of the categories which require transformation. Applying the transformation to the exampled presented in \bsref{lst:before_atomic_ref} produces a syntax tree representing the code shown in \bsref{lst:after_atomic_ref}. For each argument node the transformation goes trough the following steps:

\begin{enumerate}
	\item Determine whether the argument is passed to a \bscode{ref} or \bscode{out} parameter, if not skip any transformation
	\item If the argument is of type $T$ and the parameter is of type \bscode{atomic} $T$
	\begin{enumerate}
		\item Construct a syntax node for a local variable of type $T$ initialized using the original argument and insert it just before the method call.
		\item Replace the original argument with with a identifier equal to the name of the previously generated intermediate local variable
...
\begin{lstlisting}[label=lst:after_atomic_ref,
  ...
  public class AtomicRefExample
  {
    ...
    public static void Main()
    {
      ...
      TMInt _gen1 = new TMInt(i);
      TestMethodAtomic(ref _gen1);
      i = _gen1.Value;

      TMInt _gen2 = new TMInt(iAtomic.Value);
      TestMethodAtomic(ref _gen2);
      iAtomic.Value = _gen2.Value;

      int _gen3 = iAtomic.Value;
      TestMethod(ref _gen3);
      iAtomic.Value = _gen3;
    }
  }
\end{lstlisting}
...
Analysis has been implemented to generate an error if the original argument does not correspond to a variable as this error is otherwise lost. 

\subsection{Retry}
In \bsref{sec:sync_design} we described the design of conditional synchronization through the use of \bscode{retry}. As \bscode{retry} is a keyword employed as a statement, we need to identify all \bscode{retry} keywords and replace them with a method invocation to the static method \bscode{STMSystem.Retry} defined in the \ac{STM} library, described in \bsref{subsec:impl_retry}. The library then carries out the effect of the \bscode{retry} statement.

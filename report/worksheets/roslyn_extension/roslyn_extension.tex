...
Disallowing \bscode{ref/out} the cases where both a \bscode{atomic} and non \bscode{atomic} variable/parameter is involved removes the cases where the desired timing can not be provided. However it, for example, non longer be possible to pass a atomic integer into a \bsref{out} integer parameter such as that of \bscode{System.Int32.TryParse}. As a result the programmer is more restricted when using \bscode{atomic} variables.

The final option involves disallowing both \bscode{atomic ref/out} parameters and \bscode{atomic} variables as \bscode{ref/out} arguments. Here the distinguishing 

 
Requiring that the argument passed to a parameter of type \bscode{atomic} $T$ to be a variable of the same type. 

...
Syntax nodes represent non-terminals and are one of the main elements of syntax trees. Each syntax node is represented as a separate class that derives from a base \bscode{SyntaxNode} class.
...
Syntax tokens represent terminals and is another of the main elements of syntax trees. Opposed to syntax nodes, syntax tokes does not have any children. 

The \bscode{SyntaxToken} class is for performance reasons a struct type and therefore a value type\cite[p. 7]{ng2012roslyn}. This means that BLA

%there is only one structure for all kinds of syntax tokens, which have different 
%struct

%does not have a property for each chi

\subsubsection{Syntax Trivia}
\subsubsection{Spans}
\subsubsection{Kinds}
\subsubsection{Errors}
%Vær sikker på at det ikke allerede det blevet beskrevet.

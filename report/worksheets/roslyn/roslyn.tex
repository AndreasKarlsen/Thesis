...
For the expression ``5+5*5+5'', a typical parse tree is shown in \bsref{fig:normal_syntax_tree}, and a potential green tree is shown in \bsref{fig:green_syntax_deduplication}. The green tree is more compact and can reuse nodes because it does not store parent references and positions in source code for nodes.
%this is obtained by the factory methods used to create nodes, first checks if a given expression is already avaialble.
	%restriction
...
%Husk, inspired by blog indlæg med VSadov

%however, det er ikke alle der ser sådan ud (pga. det ikke er performance mæssigt godt at cache all subtræer)

%more about green tree reuse

%As the green tree nodes does not know parents or positions of nodes in source code, similar sub trees can be reused across several 
An advantage of the green tree 


%when you create a node with factory, it already has a similar expression

%green nodes does not know their parent 

%forklar at grønne noder kan genbruges, når man modificerer et træ og laver et nyt og man kan genbruge unaffected noder. En grøn node kan være brugt af mange forskellige syntax træer.

%figur over grøn træ og dets noder

...
The projects contained in the \bscode{CSharp} folder are shown on \bsref{fig:roslyn_csharp_overview}. The \bscode{csc} project is the C\# command line compiler, which is the starting point of a C\# compilation. It receives the arguments passed to compilation and pass them to \bscode{Csc} class located in the \bscode{csc} project using the \bscode{Run} method. \bscode{Csc} is a subtype of the \bscode{CSharpCompiler} class located in the \bscode{CSharpCodeAnalysis.Desktop} project, and it further invokes its parents \bscode{Run} method. \bscode{CSharpCompiler} is a subtype of the class \bscode{CommonCompiler} located in the \bscode{CSharpCodeAnalysis.Desktop} project inside the \bscode{Core} folder, which is the top parent. The \bscode{Run} method invokes the \bscode{RunCore} method in the same class, which contains the code that controls the overall compilation flow.

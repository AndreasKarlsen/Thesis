...
No additional restrictions is made on transactional properties marked \bscode{virtual.} Consequently the programmer can override a \bscode{virtual atomic} property supplying a non \bscode{atomic} implementation. Similarly the programmer is allowed to override a non \bscode{atomic}, \bscode{virtual} property with a \bscode{atomic} auto implemented property of the same type. This approach is flexible with regards to supplying transactional implementations of existing non transactional base classes and allows non transactional implementations based on a transactional base class, raising the orthogonality of \stmname.
...
In \stmname, a reference parameter can be marked as atomic, and becomes a \ac{TRP}. A \ac{TRP} differs from a reference parameter by being tracked in transactions. Assignments to a \ac{TRP} do not take effect outside of the transaction immediately, but when the transaction commits. This is a design choice made to enforce atomicity, as immediate changes to a \ac{TRP} would enable reading intermediate transaction states. A noteworthy detail is, that changes made to a \ac{TRP} through side-effects will be immediate, as we do not track side-effects as discussed in \bsref{sec:tracking}.
...
\end{enumerate} 
where $T$ is an argument of some type $T$, \bscode{atomic} $T$ is a \bscode{atomic} parameter of type $T$ and $T$ $\quad \rightarrow \quad$ \bscode{atomic} $T$ describes passing an argument of type $T$ into a \bscode{atomic} parameter of type $T$. When an expression is passed as an argument, it is by value as in C\#. Consequently, in the first case, assignment to the parameter will be tracked inside the method. In the second case, the atomic variable will not be tracked inside the method as the value of the \bscode{atomic} $T$ variable supplied as the argument is passed to the parameter and the variable itself. In the third case assignments to the \bscode{atomic} parameter is tracked inside the method. The tracking is however separated from that of the \bscode{atomic} $T$ variable supplied as the argument, since the value of that variable is passed to parameter and not the variable itself. Thus assignments to the parameter and argument variable are tracked separately. 
...
To increase the orthogonality of \stmname the \bscode{atomic} keyword can be used for making a variable, field, property, or parameter trackable in transactions. This enables continues use of these common C\# features, within a transactional context, including reference and output parameters.

...
where $param$-$modifier$ represents C\#'s \bscode{ref} and \bscode{out} keywords, and $type$ can be any existing or user defined type including the \bscode{dynamic} keyword, as previously stated. The \bscode{atomic} keyword can be combined with optional parameters, and follow the standard rules for optional parameters\cite[p. 46-47]{sestoft2011c}.
...
\end{enumerate} 
where $T$ is some type, \bscode{atomic} $T$ is $T$'s corresponding \ac{STM} type and $T$ $\quad \rightarrow \quad$ \bscode{atomic} $T$ describes passing an argument of type $T$ into a parameter of the \ac{STM} type corresponding to $T$.  When an expression is passed as an argument, it is by value. E.g. in the first case, the variable will be tracked inside the method, but changes to it will not have effect outside of the method. In the second case, the atomic variable will not be tracked inside the method. Thus, its semantics are as described in \bsref{subsec:stm_desgin_value_parameters}. Transactional arguments adhere to the standard rules in regards to named parameters\cite[p. 145]{csharp2013specificaiton}. Just as with the parameter modifiers \bscode{ref} and \bscode{out}, an overloaded method cannot only differ on the atomic modifier, as it does not change its type. Thus, it follows the rules defined in the specification\cite[p. 153-157]{csharp2013specificaiton}.

\subsubsection{Transactional Reference \& Output Arguments}\label{subsubsec:trans_ref_out_args}
When a variable reference is passed as \bscode{ref} or \bscode{out}, it follows the call-by-reference semantics described in \bsref{subsec:stm_desgin_ref_parameters} and \bsref{subsec:stm_desgin_out_parameters}. In the first case, a variable reference passed in to an atomic reference will result in the variable being tracked inside of the method, and assignments to it will take effect outside of the method. The in the second case, there is no tracking of the reference variable but assignments still take effect outside of the method. In the third case, the variable will continued to be tracked. The fourth case is the normal case, which follows the standard rules\cite[p. 145]{csharp2013specificaiton}. 

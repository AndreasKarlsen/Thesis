...
As described in \bsref{sec:stm_keyconcepts_example} an \ac{STM} system must offer some way of defining a transaction scope. As \stmnamesp is to be a language integrated \ac{STM} system, C\# must be extended with syntax for specifying a transaction scope.\andreas{Does this belong under transactions and variables? It seems to stick out}
...
As we seek to validate whether \ac{STM} is a valid alternative to locks by characteristics, and not by performance, we choose to use strong atomicity in our \ac{STM} system.

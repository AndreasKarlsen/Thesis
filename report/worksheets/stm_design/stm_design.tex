...
Throughout this chapter, the proposed syntax extensions is described using Extended Backus-Naur Form. The notation uses ? for signaling 0 or 1 occurrences and + signaling for 0 or more occurrences. Parentheses are used for grouping: ( item1, item2 ). Items formatted as \bscode{terminal} are terminals, while items formatted as $non-term$ are non-terminals. The examples presented are based on the syntax used in C\# Precisely\cite{sestoft2011c}. This is chosen due to its compact and readable format. As the examples serve to illustrate how the new constructs integrate with the existing language features, some elements of the syntax will not be explained in depth. A reference to where it can be found will be provided in each example. The complete C\# grammar used for parsing can be found at the C\# specification\cite{csharp2013specificaiton}.
...
No additional restrictions are made on transactional properties marked \bscode{virtual.} Consequently the programmer can override a \bscode{virtual atomic} property supplying a non \bscode{atomic} implementation. Similarly the programmer is allowed to override an non \bscode{atomic}, \bscode{virtual} property with a \bscode{atomic} auto implemented property of the same type. This approach is flexible when supplying transactional implementations of existing non transactional base classes and allows non transactional implementations based on a transactional base class, raising the orthogonality of \stmname.
...
In \stmname, an output parameter can be marked as atomic, and it becomes a \ac{TOP}. Similar to \ac{TRP}, a \ac{TOP} is tracked in transactions, and assignments in transactions take effect outside of the transaction when it commits. The reason for this is the same as for \acp{TRP}.
...
According to \cite[p. 17]{csharp2013specificaiton}: \textit{``A parameter array permits a variable number of arguments to be passed to a method''}. A parameter array is declared using the \bscode{params} modifier. Inside the method, the arguments are are represented as a array of length equal to the number of arguments supplied to \bscode{params} modified parameter array. Assignments to the parameter array are possible but this possibility is in our experience rarely used. Additionally, as only assignments to the parameter array are tracked, adding or removing items from the array, would not be tracked in transactions. Thus, the estimated value of adding this feature is close to none. Therefore, a parameter array cannot be marked as atomic in \stmname.

\subsection{Transactional Arguments}
Invoking a method requires supplying arguments to match the parameters of the method. These arguments must be in the form of an expression, or variable references combined with the \bscode{ref} or \bscode{out} parameter-modifier \cite[p. 144]{csharp2013specificaiton}.
...
To increase the orthogonality of \stmname the \bscode{atomic} keyword can be used for making a variable, field, property, or parameter trackable in transactions. This enables continued use of these common C\# features, within a transactional context, including reference and output parameters.

\stmnamesp also introduces the keywords \bscode{retry} and \bscode{orelse}, which can be used for conditional synchronization. \bscode{retry} will block the executing thread until a transactional variable previously read by the transactions is changed. The \bscode{orelse} keyword associates an \bscode{orElse} block, with an \bscode{atomic} block containing a \bscode{retry} statement. The \bscode{orElse} block will change the initial behavior of the \bscode{retry} statement in the \bscode{atomic} block. Instead of blocking execution when encountering a \bscode{retry} statement, the \bscode{orElse} block will be executed. Execution will in that case only block if the bscode{retry} statement of the last \bscode{OrElse} block associated with an \bscode{atomic} block is reached.

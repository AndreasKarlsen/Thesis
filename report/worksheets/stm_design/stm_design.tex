...
A notation similar to Extended Bacus-Naur Form is used for descriptions of the proposed syntax extensions, with ? signaling 0 or 1 and + signaling 1 or more and parentheses used for grouping as such ( item1, item2 ). Items formatted as \bscode{Non-Term} are non terminals, usually keywords, while terminals are formatted as such, $terminal$. 
...
$field$-$modifiers$ $type$ $name$ = $initial$-$value$+ ; or\\
...
\bscode{atomic}\bscode{var} $name$ = $expr$ or\\
\bscode{atomic} $type$ $var$-$decl^1$, $var$-$dcl^2$, ...; 
...
\bscode{atomic?} $param$-$modifier$? $type$ $name$ or\\
\bscode{atomic?} $type$ $name$ = $constant$

where $param$-$modifier$ represents C\#'s \bscode{ref} and \bscode{out} keywords and $constant$ is a constant specifying the parameters default value. Just as previously $type$ can be any existing or user defined type including the \bscode{dynamic} keyword. The \bscode{atomic} keyword can not be combined with the C\# \bs{params} parameter definition\cite[p. 42]{sestoft2011c}.
